...
__Option 1__: normalize with a log transformation. This will transform most right skewed distributions to be approximately normal. One way to do this is to simply log transform the training and test set in a manual, single step manner similar to:
...
```{r heat-map-missingness, fig.width=7, fig.height=5, out.width="100%", fig.cap = "Heat map of ,issing values in the raw Ames housing data."}
...
If both of these criteria are true then it is often adventageous to remove the variable from the model. For the Ames data, we do not have any zero variance predictors but there are 21 features that meet the near-zero threshold.
...
```{r normalizing}
# we can normalize all numeric features, including the response at the same time
...
For these models and modeling components, it is often a good idea to _standardize_ the features. Standardizing features includes _centering_ and _scaling_ so that numeric variables have zero mean and unit variance, which provides a common comparable unit of measure across all the variables.
...
library(caret)
one_hot <- caret::dummyVars( ~ ., ex1, fullRank = FALSE)
ex2 <- predict(one_hot, ex1)
knitr::kable(ex2) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```

This is called less than _full rank_ encoding where we retain all variables for each level of `x`.  However, this creates perfect collinearity which causes problems with some predictive modeling algorithms (e.g., ordinary linear regression and neural networks).  Alternatively, we can create full-rank one-hot encoding by dropping one of the levels (level `a` has been dropped). This is referred to as _dummy_ encoding. 
...
```{r proportion-encoding, echo=FALSE}
ames_train %>%
  count(Neighborhood) %>%
...
```{r minimize-leakage, echo=FALSE, fig.cap="Performing feature engineering pre-processing within each resample helps to minimize data leakage."}
...
By applying a handful of the preprocessing techniques discussed throughout this chapter, we were able to reduce our prediction error by approximately \$10,000.  The chapters that follow will look to see if we can continue reducing our error by applying different algorithms and feature engineering blueprints.
